...
struct Index {
    ...
    explicit Index(const string &str) {
        len = str.size();
        for (int i = 0; i < len; i++)
            s[i] = str[i];
    }
...
class bpt {
    ...
    struct Node {
        ...
        Node() {
            ...
            for (int i = 0; i < kMaxSize + 2; i++) data[i] = ValuePair();
        }
    };
...
public:
    bpt() {
        #ifdef CLEAR
            tr.initialise("file_test", 1);
        #else
            tr.initialise("file_test");
        #endif
        tr.get_info(rt, 1);
        //std::cerr << rt << '\n';
    }
    ...
    void find(const Key &x, const Val &neginf) {
        ValuePair tmp(x, neginf);
        if (!rt) {
            puts ("null");
            return;
        }
    ...
    void testprint(int nw) {
        Node tmp;
        tr.read(tmp, nw);
        std::cout << "Node " << nw << ": "; 
        for (int i = 0; i < tmp.sz; i++) {
            if (tmp.isleaf) {
                std::cout << "{data:" << tmp.data[i].b << "} ";
                continue;
            }
            std::cout << "{data:";
            std::cout << tmp.data[i].b << ", child:" << tmp.ch[i] << "} ";
        }
        puts ("");
    ...
    void del(const Key &x, const Val &y) {
        ...
        int pos = lower_bound(block, tmp);
        if (block.data[pos] == ValuePair(x, y)) return;
...
void test() {
    ...
    std::cerr << "rt:" << p.rt << '\n';
    p.find((Index)"123" , INF);
    if (p.rt) p.testprint(p.rt);
}
int main() {
    test();
    // int T;
